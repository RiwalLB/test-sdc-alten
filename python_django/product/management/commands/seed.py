from django.core.management.base import BaseCommand
from product.models import Category, InventoryStatus, Product
from ...resources.validation_resources import CURRENCY_TO_CENTS


class Command(BaseCommand):
    help = "Seed the database with initial data"

    def handle(self, *args, **kwargs):
        self.seed_categories()
        self.seed_inventory_statuses()
        self.seed_products()
        self.stdout.write(self.style.SUCCESS("Database seeded successfully."))

    def seed_categories(self):
        categories = [
            {"id": 1, "label": "Accessories"},
            {"id": 2, "label": "Fitness"},
            {"id": 3, "label": "Clothing"},
            {"id": 4, "label": "Electronics"},
        ]
        for category in categories:
            Category.objects.update_or_create(id=category["id"], defaults=category)

    def seed_inventory_statuses(self):
        inventory_statuses = [
            {"id": 1, "label": "INSTOCK"},
            {"id": 2, "label": "LOWSTOCK"},
            {"id": 3, "label": "OUTOFSTOCK"},
        ]
        for status in inventory_statuses:
            InventoryStatus.objects.update_or_create(id=status["id"], defaults=status)

    def seed_products(self):
        products = [
            {
                "id": 1000,
                "code": "f230fh0g3",
                "name": "Bamboo Watch",
                "description": "Product Description",
                "image": "bamboo-watch.jpg",
                "price": 65,
                "category_id": 1,
                "quantity": 24,
                "inventory_status_id": 1,
                "rating": 5,
            },
            {
                "id": 1001,
                "code": "nvklal433",
                "name": "Black Watch",
                "description": "Product Description",
                "image": "black-watch.jpg",
                "price": 72,
                "category_id": 1,
                "quantity": 61,
                "inventory_status_id": 1,
                "rating": 4,
            },
            {
                "id": 1002,
                "code": "zz21cz3c1",
                "name": "Blue Band",
                "description": "Product Description",
                "image": "blue-band.jpg",
                "price": 79,
                "category_id": 2,
                "quantity": 2,
                "inventory_status_id": 2,
                "rating": 3,
            },
            {
                "id": 1003,
                "code": "244wgerg2",
                "name": "Blue T-Shirt",
                "description": "Product Description",
                "image": "blue-t-shirt.jpg",
                "price": 29,
                "category_id": 3,
                "quantity": 25,
                "inventory_status_id": 1,
                "rating": 5,
            },
            {
                "id": 1004,
                "code": "h456wer53",
                "name": "Bracelet",
                "description": "Product Description",
                "image": "bracelet.jpg",
                "price": 15,
                "category_id": 1,
                "quantity": 73,
                "inventory_status_id": 1,
                "rating": 4,
            },
            {
                "id": 1005,
                "code": "av2231fwg",
                "name": "Brown Purse",
                "description": "Product Description",
                "image": "brown-purse.jpg",
                "price": 120,
                "category_id": 1,
                "quantity": 0,
                "inventory_status_id": 3,
                "rating": 4,
            },
            {
                "id": 1006,
                "code": "bib36pfvm",
                "name": "Chakra Bracelet",
                "description": "Product Description",
                "image": "chakra-bracelet.jpg",
                "price": 32,
                "category_id": 1,
                "quantity": 5,
                "inventory_status_id": 2,
                "rating": 3,
            },
            {
                "id": 1007,
                "code": "mbvjkgip5",
                "name": "Galaxy Earrings",
                "description": "Product Description",
                "image": "galaxy-earrings.jpg",
                "price": 34,
                "category_id": 1,
                "quantity": 23,
                "inventory_status_id": 1,
                "rating": 5,
            },
            {
                "id": 1008,
                "code": "vbb124btr",
                "name": "Game Controller",
                "description": "Product Description",
                "image": "game-controller.jpg",
                "price": 99,
                "category_id": 4,
                "quantity": 2,
                "inventory_status_id": 2,
                "rating": 4,
            },
            {
                "id": 1009,
                "code": "cm230f032",
                "name": "Gaming Set",
                "description": "Product Description",
                "image": "gaming-set.jpg",
                "price": 299,
                "category_id": 4,
                "quantity": 63,
                "inventory_status_id": 1,
                "rating": 3,
            },
            {
                "id": 1010,
                "code": "plb34234v",
                "name": "Gold Phone Case",
                "description": "Product Description",
                "image": "gold-phone-case.jpg",
                "price": 24,
                "category_id": 1,
                "quantity": 0,
                "inventory_status_id": 3,
                "rating": 4,
            },
            {
                "id": 1011,
                "code": "4920nnc2d",
                "name": "Green Earbuds",
                "description": "Product Description",
                "image": "green-earbuds.jpg",
                "price": 89,
                "category_id": 4,
                "quantity": 23,
                "inventory_status_id": 1,
                "rating": 4,
            },
            {
                "id": 1012,
                "code": "250vm23cc",
                "name": "Green T-Shirt",
                "description": "Product Description",
                "image": "green-t-shirt.jpg",
                "price": 49,
                "category_id": 3,
                "quantity": 74,
                "inventory_status_id": 1,
                "rating": 5,
            },
            {
                "id": 1013,
                "code": "fldsmn31b",
                "name": "Grey T-Shirt",
                "description": "Product Description",
                "image": "grey-t-shirt.jpg",
                "price": 48,
                "category_id": 3,
                "quantity": 0,
                "inventory_status_id": 3,
                "rating": 3,
            },
            {
                "id": 1014,
                "code": "waas1x2as",
                "name": "Headphones",
                "description": "Product Description",
                "image": "headphones.jpg",
                "price": 175,
                "category_id": 4,
                "quantity": 8,
                "inventory_status_id": 2,
                "rating": 5,
            },
            {
                "id": 1015,
                "code": "vb34btbg5",
                "name": "Light Green T-Shirt",
                "description": "Product Description",
                "image": "light-green-t-shirt.jpg",
                "price": 49,
                "category_id": 3,
                "quantity": 34,
                "inventory_status_id": 1,
                "rating": 4,
            },
            {
                "id": 1016,
                "code": "k8l6j58jl",
                "name": "Lime Band",
                "description": "Product Description",
                "image": "lime-band.jpg",
                "price": 79,
                "category_id": 2,
                "quantity": 12,
                "inventory_status_id": 1,
                "rating": 3,
            },
            {
                "id": 1017,
                "code": "v435nn85n",
                "name": "Mini Speakers",
                "description": "Product Description",
                "image": "mini-speakers.jpg",
                "price": 85,
                "category_id": 3,
                "quantity": 42,
                "inventory_status_id": 1,
                "rating": 4,
            },
            {
                "id": 1018,
                "code": "09zx9c0zc",
                "name": "Painted Phone Case",
                "description": "Product Description",
                "image": "painted-phone-case.jpg",
                "price": 56,
                "category_id": 1,
                "quantity": 41,
                "inventory_status_id": 1,
                "rating": 5,
            },
            {
                "id": 1019,
                "code": "mnb5mb2m5",
                "name": "Pink Band",
                "description": "Product Description",
                "image": "pink-band.jpg",
                "price": 79,
                "category_id": 2,
                "quantity": 63,
                "inventory_status_id": 1,
                "rating": 4,
            },
            {
                "id": 1020,
                "code": "r23fwf2w3",
                "name": "Pink Purse",
                "description": "Product Description",
                "image": "pink-purse.jpg",
                "price": 110,
                "category_id": 1,
                "quantity": 0,
                "inventory_status_id": 3,
                "rating": 4,
            },
            {
                "id": 1021,
                "code": "pxpzczo23",
                "name": "Purple Band",
                "description": "Product Description",
                "image": "purple-band.jpg",
                "price": 79,
                "category_id": 2,
                "quantity": 6,
                "inventory_status_id": 2,
                "rating": 3,
            },
            {
                "id": 1022,
                "code": "2c42cb5cb",
                "name": "Purple Gemstone Necklace",
                "description": "Product Description",
                "image": "purple-gemstone-necklace.jpg",
                "price": 45,
                "category_id": 1,
                "quantity": 62,
                "inventory_status_id": 1,
                "rating": 4,
            },
            {
                "id": 1023,
                "code": "5k43kkk23",
                "name": "Purple T-Shirt",
                "description": "Product Description",
                "image": "purple-t-shirt.jpg",
                "price": 49,
                "category_id": 3,
                "quantity": 2,
                "inventory_status_id": 2,
                "rating": 5,
            },
            {
                "id": 1024,
                "code": "lm2tny2k4",
                "name": "Shoes",
                "description": "Product Description",
                "image": "shoes.jpg",
                "price": 64,
                "category_id": 3,
                "quantity": 0,
                "inventory_status_id": 1,
                "rating": 4,
            },
            {
                "id": 1025,
                "code": "nbm5mv45n",
                "name": "Sneakers",
                "description": "Product Description",
                "image": "sneakers.jpg",
                "price": 78,
                "category_id": 3,
                "quantity": 52,
                "inventory_status_id": 1,
                "rating": 4,
            },
            {
                "id": 1026,
                "code": "zx23zc42c",
                "name": "Teal T-Shirt",
                "description": "Product Description",
                "image": "teal-t-shirt.jpg",
                "price": 49,
                "category_id": 3,
                "quantity": 3,
                "inventory_status_id": 2,
                "rating": 3,
            },
            {
                "id": 1027,
                "code": "acvx872gc",
                "name": "Yellow Earbuds",
                "description": "Product Description",
                "image": "yellow-earbuds.jpg",
                "price": 89,
                "category_id": 4,
                "quantity": 35,
                "inventory_status_id": 1,
                "rating": 3,
            },
            {
                "id": 1028,
                "code": "tx125ck42",
                "name": "Yoga Mat",
                "description": "Product Description",
                "image": "yoga-mat.jpg",
                "price": 20,
                "category_id": 2,
                "quantity": 15,
                "inventory_status_id": 1,
                "rating": 5,
            },
            {
                "id": 1029,
                "code": "gwuby345v",
                "name": "Yoga Set",
                "description": "Product Description",
                "image": "yoga-set.jpg",
                "price": 20,
                "category_id": 2,
                "quantity": 25,
                "inventory_status_id": 1,
                "rating": 8,
            },
        ]

        for product in products:
            category = Category.objects.get(id=product["category_id"])
            inventory_status = InventoryStatus.objects.get(
                id=product["inventory_status_id"]
            )
            Product.objects.update_or_create(
                id=product["id"],
                defaults={
                    "code": product["code"],
                    "name": product["name"],
                    "description": product["description"],
                    "image": product["image"],
                    "price": product["price"] * CURRENCY_TO_CENTS,
                    "quantity": product["quantity"],
                    "category": category,
                    "inventory_status": inventory_status,
                    "rating": product["rating"],
                },
            )
